...
module SequestLike =
    ...
    let predictSEQUEST  (aa:seq<AminoAcids.AminoAcid>) (maxcharge:float) =
        ...
        let rec recloop (ions) (charge) (b'bNbH_list:List<float*float*float>) (y'yNyH_list:List<float*float*float>) =
            ...
            | (b,bN,bH)::restb,(y,yN,yH)::resty -> let b'  = (createPeak IonTypes.B        (BioFSharp.Mass.toMZ b charge)  (predictIntensitySimpleModel IonTypes.B charge) nan)
                                                   let bN' = (createPeak IonTypes.BlossNH3 (BioFSharp.Mass.toMZ bN charge) (predictIntensitySimpleModel IonTypes.BlossNH3 charge) nan)
                                                   let bH' = (createPeak IonTypes.BlossH2O (BioFSharp.Mass.toMZ bH charge) (predictIntensitySimpleModel IonTypes.BlossH2O charge) nan)
                                                                            
                                                   let y'  = (createPeak IonTypes.Y        (BioFSharp.Mass.toMZ y charge)  (predictIntensitySimpleModel IonTypes.Y charge) nan)
                                                   let yN' = (createPeak IonTypes.YlossNH3 (BioFSharp.Mass.toMZ yN charge) (predictIntensitySimpleModel IonTypes.YlossNH3 charge) nan)
                                                   let yH' = (createPeak IonTypes.YlossH2O (BioFSharp.Mass.toMZ yH charge) (predictIntensitySimpleModel IonTypes.YlossH2O charge) nan)
